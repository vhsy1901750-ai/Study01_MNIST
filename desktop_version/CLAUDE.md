...
## MNIST에 없는 글씨체를 합성해 넣는다

`make_flat_six.py`는 위가 평평한 6과 같은 손의 5를 그리고, `flat_dataset.py`가 각 3,000장을 MNIST에 섞는다.

MNIST의 6은 전부 위쪽이 둥글게 흘러내리는 미국식 필체다. 가로줄로 시작해 왼쪽을 타고 내려오는 6은 한 장도 없어서, 그렇게 쓴 6을 모델이 5로 읽었다. 회전이나 확대 같은 증강으로는 없는 획 모양을 만들어낼 수 없어 합성이 필요했다.

6만 넣으면 이번에는 5를 6으로 잘못 읽게 되므로 5도 함께 만든다. 글씨체를 고쳤다면 `python make_flat_six.py`로 `flat_sample.png`를 만들어 모양을 눈으로 확인한 뒤 재학습한다. 이 그림은 확인용이므로 커밋하지 않는다.
